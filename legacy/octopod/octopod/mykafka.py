...
def consume_and_produce(consume_names, deserializer, callback, pub_names, serializer=None):
    """
    Asynchronously consumes from topic `consume_name`, spawns a thread
    to deal with the message using `callback` and then takes `callback`'s
    result and produces it asynchronously.
...
def consume(consume_names, deserializer=None):
    """
    Synchronously consumes from topic `consume_name` and, if `deserializer` is not None,
...
def produce(pub_names, key, item, serializer=None):
    """
    Synchronously producees `item` to each topic in `pub_names`. If `serializer` is not None,
