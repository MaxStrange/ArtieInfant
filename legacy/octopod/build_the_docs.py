...
if __name__ == "__main__":
    ...
    shutil.copyfile(os.sep.join([home, "octopod", "octopod.py"]), os.sep.join([apipath, "octopod.py"]))
    ...
    with open(os.sep.join([apipath, "make.bat"])) as f:
        lines = [line for line in f]
    # Need to remove the check for the sphinx module from the autogenerated batch file, as it doesn't work
    start_line = None
    end_line = None
    for i, line in enumerate(lines):
        if line.startswith("\tset SPHINXBUILD="):
            lines[i] = "\tset SPHINXBUILD=" + python_interp + " -m sphinx" + os.linesep
        elif line.startswith("REM Check if sphinx-build is available"):
            start_line = i
        elif start_line is not None and end_line is None and line.startswith(")"):
            end_line = i

    # If end_line is None, we are dealing with an updated Sphinx, so
    # we don't need this work around
