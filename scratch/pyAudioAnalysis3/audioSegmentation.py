...
def silenceRemoval(x, Fs, stWin, stStep, smoothWindow=0.5, Weight=0.5, plot=False):
    ...
    if plot:
        ...
        if timeX.shape[0] > x.shape[0]:
            timeX = timeX[:x.shape[0]]
...
def speakerDiarization(x, Fs, numOfSpeakers, mtSize=2.0, mtStep=0.2, stWin=0.05, LDAdim=35, PLOT=False):
    '''
    ARGUMENTS:
        - x                the raw audio data
